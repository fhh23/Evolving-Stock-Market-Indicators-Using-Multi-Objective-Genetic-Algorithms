...
public class InputOutput {
    ...
    */
    ...
    public static void displayExtremePoints(
            ...
            Individual[] extremePoints) {
        System.out.println("-----------------------------------");
        System.out.format("%s%n", populationLabel);
        System.out.println("-----------------------------------");
        for (Individual individual : extremePoints) {
            System.out.print("(");
            for (int i = 0; i < optimizationProblem.objectives.length; i++) {
    ...
    public static void displayReferenceDirections(String populationLabel, List<ReferenceDirection> referenceDirections) {
        ...
        for (int i = 0; i < referenceDirections.size(); i++) {
            ...
            for (int j = 0; j < direction.length; j++) {
                System.out.format("%5.2f", direction[j]);
    ...
    public static void dumpCurrentGeneration_proposed(
            ...
            IndividualEvaluator individualEvaluator) throws IOException, EvaluationException {
    ...
    public static void dumpMatlabPlottinScriptFor3dPoints(String filePath, IndividualEvaluator evaluator, Individual[] individuals) throws IOException {
        PrintWriter printer = null;
        try {
            printer = new PrintWriter(filePath);
            ...
            yDim.append("];");
            zDim.append("];");
            printer.println(xDim.toString());
            printer.println(yDim.toString());
            printer.println(zDim.toString());
            printer.println("plot3(xDim, yDim, zDim, 'ok', 'MarkerSize', 3, 'MarkerFaceColor','k');");
            //printer.println("scatter3(xDim,yDim,zDim, 'filled', 'k');");
            // The following three lines will vary according to the algorithm
            try {
                double obj0MinFactor = evaluator.getIdealPoint()[0] > 0 ? 0.75 : 1.25;
                double obj0MaxFactor = evaluator.getReferencePoint()[0] > 0 ? 1.25 : 0.75;
                double obj1MinFactor = evaluator.getIdealPoint()[1] > 0 ? 0.75 : 1.25;
                double obj1MaxFactor = evaluator.getReferencePoint()[1] > 0 ? 1.25 : 0.75;
                double obj2MinFactor = evaluator.getIdealPoint()[2] > 0 ? 0.75 : 1.25;
                double obj2MaxFactor = evaluator.getReferencePoint()[2] > 0 ? 1.25 : 0.75;
                printer.format("xlim([%f %f])%n", obj0MinFactor * evaluator.getIdealPoint()[0], obj0MaxFactor * evaluator.getReferencePoint()[0]);
                printer.format("ylim([%f %f])%n", obj1MinFactor * evaluator.getIdealPoint()[1], obj1MaxFactor * evaluator.getReferencePoint()[1]);
                printer.format("zlim([%f %f])%n", obj2MinFactor * evaluator.getIdealPoint()[2], obj2MaxFactor * evaluator.getReferencePoint()[2]);
            } catch (UnsupportedOperationException ex) {
                System.out.println("Limits cannot be specified in MATLAB "
                        + "script because the evaluator does not specify "
                        + "Ideal point and/or reference point.");
            }
            printer.format("view(125,25)%n");
            printer.format("grid on");
        } finally {
            if (printer != null) {
                printer.close();
            }
     ...
     */
    public static void printMatlabCode2D(PrintWriter printer, IndividualEvaluator evaluator, VirtualIndividual[]  
        ... individualsArrays) {
        Color[] colors = {
            Color.BLACK, Color.RED, Color.BLUE, Color.GREEN,
            Color.YELLOW, Color.gray, Color.pink};
        int count = 0;
        printer.println("hold on");
        for (VirtualIndividual[] individuals : individualsArrays) {
            ...
            xBuilder.append("];");
            yBuilder.append("];");
            printer.println(xBuilder.toString());
            printer.println(yBuilder.toString());
            ...
            printer.println(plot);
            String clearXCommand = String.format("clear X%02d", count);
            String clearYCommand = String.format("clear Y%02d", count);
            printer.println(clearXCommand);
            printer.println(clearYCommand);
            count++;
        }
        boolean connectCorrespondingPoints = true;
        if (connectCorrespondingPoints) {
            for (int i = 0; i < individualsArrays.length - 1; i++) {
                printer.format("for i = 1:size(X%02d,2)%n", i);
                printer.format("    plot([X%02d(i),X%02d(i)],[Y%02d(i),Y%02d(i)])%n", i, i + 1, i, i + 1);
                printer.println("end");
            }
        }
        printer.println("hold off");
        try {
            double obj0MinFactor = evaluator.getIdealPoint()[0] > 0 ? 0.75 : 1.25;
            double obj0MaxFactor = evaluator.getReferencePoint()[0] > 0 ? 1.25 : 0.75;
            double obj1MinFactor = evaluator.getIdealPoint()[1] > 0 ? 0.75 : 1.25;
            double obj1MaxFactor = evaluator.getReferencePoint()[1] > 0 ? 1.25 : 0.75;
            printer.println(String.format("xlim([%5.2f %5.2f])", obj0MinFactor * evaluator.getIdealPoint()[0], obj0MaxFactor * evaluator.getReferencePoint()[0]));
            printer.println(String.format("ylim([%5.2f %5.2f])", obj1MinFactor * evaluator.getIdealPoint()[1], obj1MaxFactor * evaluator.getReferencePoint()[1]));
        } catch (UnsupportedOperationException ex) {
            System.out.println("Limits cannot be specified in MATLAB "
                    + "script because the evaluator does not specify "
                    + "Ideal point and/or reference point.");
    ...
    public static void dumpMatlabPlottinScriptFor2dPoints(String filePath, IndividualEvaluator evaluator, VirtualIndividual[]   ... individualsArray) throws FileNotFoundException {
        PrintWriter printer = null;
        try {
            printer = new PrintWriter(filePath);
            printMatlabCode2D(printer, evaluator, individualsArray);
        } finally {
            if (printer != null) {
                printer.close();
            }
    ...
    public static VirtualIndividual[] readIndividualsFromFile(File file) throws IOException {
    ...
    public static void dumpPopulationObjectiveSpace_Undecorated_SpaceSeparated(
            OptimizationProblem optimizationProblem,
            Individual[] individuals,
            String filePath)
    ...
    public static void dumpPopulationRealDecisionSpace_Undecorated_SpaceSeparated(
            OptimizationProblem optimizationProblem,
            Individual[] individuals,
            String filePath)
            throws FileNotFoundException {
        ...
        try {
            ...
            for (int i = 0; i < individuals.length; i++) {
                for (int j = 0; j < optimizationProblem.variablesSpecs.length; j++) {
                    ...
                    printer.format("%7.7f", individuals[i].real[j]);
    ...
    public static void dumpMetaData(String metaDataFilePath, Individual[] individuals, double[] idealPoint, double[] intercepts) throws FileNotFoundException {
        PrintWriter printer = null;
        try {
