...
if ( avgFitnessPerGenerationGraphs || ...
    ...
    if (avgFitnessPerGenerationGraphs)
        ...
        figure;
        yvalues = objectiveValuesPerGenerationData(:, 4) ./ objectiveValuesPerGenerationData(:, 1);
        plot(generations, yvalues, '-o');
        title('Average Objective 3 Value Per Generation', 'FontSize', 14);
        xlabel('Generation Number', 'FontSize', 14);
        ylabel('Objective 2 Value', 'FontSize', 14);
    end
    
    if ( paretoFrontFinalPopulationGraphAllRuns )
        finalPopulationObjectiveValues(:,1:2) = finalPopulationObjectiveValues .* (-1);
        ...
        ylabel('Objective 2 Value', 'FontSize', 14);
        zlabel('Objective 3 Value', 'FontSize', 14);
    end
    ...
    if ( objValueSummaryStatisticsAllRuns )
        ...
        objValueSummaryStatisticsTable(3, :) = [ meanValues(3) minValues(3) maxValues(3) ];
    end
